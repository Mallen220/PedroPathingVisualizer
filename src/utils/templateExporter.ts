...
export class TemplateEngine {
  ...
  private processBlock(content: string, context: Context): string {
    ...
    while (currentIndex < content.length) {
      ...
      if (type === "var") {
        ...
        const varExpression = content
          .substring(currentIndex + 2, endVar)
          .trim();
        ...
        const tagContent = content
          .substring(currentIndex + 2, endBlockTag)
          .trim();
        ...
        if (tagName === "if") {
          ...
          const blockEndIndex = this.findMatchingCloseTag(
            content,
  ...
  private evaluateExpression(expr: string, context: Context): any {
    ...
    // Resolve all non-operator tokens first
  ...
  private evaluateComplexPath(fullPath: string, context: Context): any {
    ...
    const parseIdentifier = () => {
      const start = cursor;
      while (
        cursor < fullPath.length &&
        /[a-zA-Z0-9_$]/.test(fullPath[cursor])
      ) {
        cursor++;
      }

...
import { loadTrajectoryFromFile, downloadTrajectory, updateRobotImageDisplay } from './file';
    ...
    describe('loadTrajectoryFromFile', () => {
        it('parses valid .pp file correctly', () => {
            const mockData: SaveData = {
                startPoint: { x: 0, y: 0, heading: 'constant', degrees: 0, startDeg: 0, endDeg: 0, reversed: false, reverse: false, type: 'point' },
                lines: [],
                shapes: []
            };

            const file = new File([JSON.stringify(mockData)], 'test.pp', { type: 'application/json' });
            const mockEvent = {
                target: {
                    files: [file]
                }

...
describe("Geometry Properties (Autogenerated)", () => {
  ...
  describe("polygonCenter", () => {
    it("center of a single point is the point itself", () => {
      fc.assert(
        fc.property(basePoint, (p) => {
          const center = polygonCenter([p]);
          expect(center[0]).toBeCloseTo(p.x);
          expect(center[1]).toBeCloseTo(p.y);
        }),
      );
    });
  });

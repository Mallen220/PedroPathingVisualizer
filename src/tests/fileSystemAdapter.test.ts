...
describe("Directory Settings & File Handlers", () => {
  ...
  describe("directorySettings", () => {
    it("should load directory settings correctly when file exists", async () => {
      ...
      mockElectronAPI.readFile.mockResolvedValue(JSON.stringify({ autoPathsDirectory: "/test/dir" }));

      // Dynamic import
      const { loadDirectorySettings } = await import("../utils/directorySettings");
      ...
      expect(mockElectronAPI.readFile).toHaveBeenCalledWith("/app/data/directory-settings.json");
    });
    ...
    it("should save directory settings correctly", async () => {
      ...
      const { saveDirectorySettings } = await import("../utils/directorySettings");
    ...
    it("should get saved auto paths directory", async () => {
        mockElectronAPI.getAppDataPath.mockResolvedValue("/app/data");
        mockElectronAPI.fileExists.mockResolvedValue(true);
        mockElectronAPI.readFile.mockResolvedValue(JSON.stringify({ autoPathsDirectory: "/saved/dir" }));

        const { getSavedAutoPathsDirectory } = await import("../utils/directorySettings");
    ...
    it("should save auto paths directory", async () => {
        mockElectronAPI.getAppDataPath.mockResolvedValue("/app/data");
        mockElectronAPI.fileExists.mockResolvedValue(true);
        mockElectronAPI.readFile.mockResolvedValue(JSON.stringify({ autoPathsDirectory: "/old/dir" }));

        const { saveAutoPathsDirectory } = await import("../utils/directorySettings");
  ...
  describe("fileHandlers", () => {
      describe("loadRecentFile", () => {
          it("should alert if electronAPI is missing", async () => {
              ...
              expect(alertMock).toHaveBeenCalledWith("Cannot load files in this environment");
          });
          ...
          it("should load file if it exists", async () => {
              mockElectronAPI.fileExists.mockResolvedValue(true);
              mockElectronAPI.readFile.mockResolvedValue('{"startPoint": { "x":0, "y":0 }}');
              const { loadRecentFile } = await import("../utils/fileHandlers");

  ...
  describe("generateJavaCode", () => {
    ...
    it("should handle wait events in sequence when provided", async () => {
        const sequence: SequenceItem[] = [
            { kind: "wait", durationMs: 500, eventMarkers: [{ name: "waitMarker", position: 0.5 }] } as any
        ];
        ...
        expect(code).toContain('NamedCommands.registerCommand("waitMarker", yourwaitMarkerCommand)');
    });
  });

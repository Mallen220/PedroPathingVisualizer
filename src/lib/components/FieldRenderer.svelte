...
<script lang="ts">
  ...
  // AnnotationOverlay is now managed via fieldOverlayRegistry
  import ContextMenu from "./ContextMenu.svelte";
  import { fieldOverlayRegistry, contextMenuRegistry } from "../registries";
  ...
  onMount(() => {
    ...
    // Event Listeners
    two.renderer.domElement.addEventListener("contextmenu", (e: MouseEvent) => {
      // Check if we have registered items
      const menuItems = get(contextMenuRegistry);
      ...
      const inchX = x.invert(transformed.x);
      const inchY = y.invert(transformed.y);

      if (!currentElem && isMouseOverField) {
        // Map registry items to context menu format
        const items = menuItems.map((item) => ({
          label: item.label,
          icon: item.icon,
          destructive: item.destructive,
          action: () => item.action(inchX, inchY),
        }));
...
>
  ...
  >
    ...
    {#if !$isPresentationMode && !isDiffMode}
      {#each $fieldOverlayRegistry as overlay (overlay.id)}
        <svelte:component
